...
export default function AuthenticatedLayout({ header, children }: PropsWithChildren<{ header?: ReactNode }>) {
    ...
    const navItems = (() => {
        ...
        return [
            { name: 'resident.dashboard', label: 'Dashboard', icon: <LayoutDashboardIcon className="h-4 w-4" /> },
            { name: 'resident.tickets.index', label: 'Laporan Saya', icon: <FileTextIcon className="h-4 w-4" /> },
        ];
    })();
