    ...
    return <AuthenticatedLayout header={<h2 className="text-xl font-semibold tracking-tight">Manajemen Tukang</h2>}><Head title="Manajemen Tukang" />
        <div className="mx-auto max-w-7xl space-y-6 p-6 lg:p-8">
            ...
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><UsersRound className="size-5" /> Daftar Tukang</CardTitle><CardDescription>{technicians.length} akun terdaftar.</CardDescription></CardHeader><CardContent>
                <Table><TableHeader><TableRow className="bg-primary/5"><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Nama</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Username</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">WhatsApp</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Tiket</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Status</TableHead><TableHead className="text-right text-xs font-bold uppercase tracking-wider text-primary">Aksi</TableHead></TableRow></TableHeader><TableBody>
                    {rows.map((technician) => <TableRow key={technician.id}><TableCell className="font-medium">{technician.name}</TableCell><TableCell>{technician.username}</TableCell><TableCell>{technician.phone_number}</TableCell><TableCell>{technician.assigned_tickets_count}</TableCell><TableCell><Badge variant={technician.is_active ? 'default' : 'secondary'} className="rounded-full">{technician.is_active ? 'Aktif' : 'Nonaktif'}</Badge></TableCell><TableCell className="text-right"><div className="flex justify-end gap-2"><Button size="icon" className="size-8" variant="action-purple" onClick={() => openEdit(technician)} aria-label="Ubah"><Pencil className="size-4" /></Button><Button size="icon" className="size-8" variant="action-purple" disabled={toggling === technician.id} onClick={() => toggle(technician)} aria-label={technician.is_active ? 'Nonaktifkan' : 'Aktifkan'}><Power className="size-4" /></Button><Button size="icon" className="size-8" variant="action-red" disabled={technician.assigned_tickets_count > 0} title={technician.assigned_tickets_count > 0 ? 'Teknisi dengan riwayat tiket tidak dapat dihapus. Nonaktifkan akun sebagai gantinya.' : 'Hapus'} onClick={() => setDeleting(technician)} aria-label="Hapus"><Trash2 className="size-4" /></Button></div></TableCell></TableRow>)}
                    {rows.length === 0 && <TableRow><TableCell colSpan={6} className="h-48 text-center"><div className="flex flex-col items-center justify-center space-y-3"><div className="rounded-full bg-primary/10 p-3"><UsersRound className="h-6 w-6 text-primary" /></div><p className="text-sm font-medium text-slate-900">Belum ada tukang terdaftar</p><p className="text-xs text-slate-500">Klik "Tambah Tukang" untuk mendaftarkan akun baru.</p></div></TableCell></TableRow>}
                </TableBody></Table>
            </CardContent></Card>
            <Dialog open={adding} onOpenChange={setAdding}><DialogContent><DialogHeader><DialogTitle>Tambah Tukang</DialogTitle><DialogDescription>Buat akun baru untuk menerima work order.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); create.post(route('admin.technicians.store'), { onSuccess: () => { create.reset(); setAdding(false); toast.success('Akun teknisi berhasil dibuat.'); }, onError: () => toast.error('Akun teknisi gagal dibuat.') }); }}><Field label="Nama" value={create.data.name} onChange={(value) => create.setData('name', value)} error={create.errors.name} /><Field label="Username" value={create.data.username} onChange={(value) => create.setData('username', value)} error={create.errors.username} /><Field label="WhatsApp" type="tel" value={create.data.phone_number} onChange={(value) => create.setData('phone_number', value)} error={create.errors.phone_number} /><Field label="Password sementara" type="password" value={create.data.password} onChange={(value) => create.setData('password', value)} error={create.errors.password} /><Button className="w-full" disabled={create.processing}>{create.processing ? 'Menyimpan...' : 'Simpan'}</Button></form></DialogContent></Dialog>
             ...
             <Dialog open={!!deleting} onOpenChange={(open) => !open && setDeleting(null)}>{deleting && <DialogContent><DialogHeader><DialogTitle>Hapus akun teknisi?</DialogTitle><DialogDescription>Akun {deleting.name} akan dihapus. Pastikan tidak ada kebutuhan operasional yang masih bergantung pada akun ini.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setDeleting(null)}>Batal</Button><Button variant="destructive" disabled={update.processing} onClick={() => { update.delete(route('admin.technicians.destroy', deleting.id), { onSuccess: () => { setDeleting(null); toast.success('Akun teknisi berhasil dihapus.'); }, onError: () => toast.error('Akun teknisi gagal dihapus.') }); }}>Hapus Akun</Button></DialogFooter></DialogContent>}</Dialog>
        </div>
    </AuthenticatedLayout>;
}
