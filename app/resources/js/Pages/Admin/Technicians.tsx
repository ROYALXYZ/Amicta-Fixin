    ...
    return <AuthenticatedLayout header={<h2 className="text-xl font-semibold tracking-tight">Manajemen Tukang</h2>}><Head title="Manajemen Tukang" />
        <div className="mx-auto max-w-7xl space-y-6 p-6 lg:p-8">
            ...
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><UsersRound className="size-5" /> Daftar Tukang</CardTitle><CardDescription>{technicians.length} akun terdaftar.</CardDescription></CardHeader><CardContent>
                <Table><TableHeader><TableRow className="bg-primary/5 hover:bg-primary/5"><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Nama</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Username</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">WhatsApp</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Tiket</TableHead><TableHead className="text-xs font-bold uppercase tracking-wider text-primary">Status</TableHead><TableHead className="text-right text-xs font-bold uppercase tracking-wider text-primary">Aksi</TableHead></TableRow></TableHeader><TableBody>
                    {rows.map((technician) => <TableRow key={technician.id} className="transition-colors duration-200 hover:bg-primary/5"><TableCell className="font-medium">{technician.name}</TableCell><TableCell>{technician.username}</TableCell><TableCell>{technician.phone_number}</TableCell><TableCell>{technician.assigned_tickets_count}</TableCell><TableCell><Badge variant={technician.is_active ? 'default' : 'secondary'} className="rounded-full">{technician.is_active ? 'Aktif' : 'Nonaktif'}</Badge></TableCell><TableCell className="text-right"><div className="flex justify-end gap-2"><Button size="icon" className="size-8" variant="action-purple" onClick={() => openEdit(technician)} aria-label="Ubah"><Pencil className="size-4" /></Button><Button size="icon" className="size-8" variant="action-purple" disabled={toggling === technician.id} onClick={() => toggle(technician)} aria-label={technician.is_active ? 'Nonaktifkan' : 'Aktifkan'}><Power className="size-4" /></Button><Button size="icon" className="size-8" variant="action-red" onClick={() => { if (confirm(`Hapus akun ${technician.name}?`)) update.delete(route('admin.technicians.destroy', technician.id)); }} aria-label="Hapus"><Trash2 className="size-4" /></Button></div></TableCell></TableRow>)}
