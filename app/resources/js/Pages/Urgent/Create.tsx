                        ...
                        <CardContent className="pt-6">
                            ...
                            <form onSubmit={submit} className="space-y-6">
                                <div className="grid gap-5 sm:grid-cols-2"><Field label="Nama Anda" error={form.errors.reporter_name}><Input value={form.data.reporter_name} onChange={(event) => form.setData('reporter_name', event.target.value)} required /></Field><Field label="Nomor WhatsApp" error={form.errors.reporter_phone}><Input type="tel" placeholder="08xxxxxxxxxx" value={form.data.reporter_phone} onChange={(event) => form.setData('reporter_phone', event.target.value)} required /></Field></div>
                                 <div className="grid gap-5 border-t border-slate-100 pt-5 sm:grid-cols-2"><Field label="Gedung / Area" error={form.errors.building_id}><Select value={form.data.building_id} onValueChange={(value) => { form.setData('building_id', value); form.setData('unit_id', ''); }}><SelectTrigger><SelectValue placeholder="Pilih gedung" /></SelectTrigger><SelectContent>{buildings.map((building) => <SelectItem key={building.id} value={String(building.id)}>{building.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Unit" error={form.errors.unit_id}><Select value={form.data.unit_id} onValueChange={(value) => form.setData('unit_id', value)}><SelectTrigger><SelectValue placeholder="Pilih unit" /></SelectTrigger><SelectContent>{units.map((unit) => <SelectItem key={unit.id} value={String(unit.id)}>{unit.number}</SelectItem>)}</SelectContent></Select></Field></div>
                                <Field label="Jenis Masalah" error={form.errors.issue_category_name}><Input placeholder="Contoh: AC bocor, listrik mati, pintu rusak" value={form.data.issue_category_name} onChange={(event) => form.setData('issue_category_name', event.target.value)} required /></Field>
                                 <Field label="Deskripsi masalah" error={form.errors.description}><Textarea className="min-h-32 resize-y" placeholder="Jelaskan lokasi dan kondisi kerusakan..." value={form.data.description} onChange={(event) => form.setData('description', event.target.value)} required /></Field>
